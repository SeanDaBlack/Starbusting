...
def generate_account(driver, fake_identity):
    ...
    for key in XPATHS_1.keys():
        if key in ('email', 'email-retype'):
            ...
            info = fake_identity['first_name'] + fake_identity['last_name'] + str(random.randint(0,10000))
    ...
    try:
        element_present = expected_conditions.presence_of_element_located((By.ID, 'et-ef-content-ftf-gp-j_id_id16pc9-page_0-cpi-cfrmsub-frm-dv_cs_candidate_personal_info_FirstName'))
...
def application_part_5(driver, random_city, fake_identity):
    for key in XPATH_VOL.keys():
        if key in ('VOL_NAME'):
            driver.find_element_by_xpath(XPATH_VOL.get(key)).send_keys(fake_identity['first_name'] + " " + fake_identity['last_name'])
        elif key in ('VOL_DATE'):
            driver.find_element_by_xpath(XPATH_VOL.get(key)).send_keys(today.strftime("%m/%d/%y"))
        elif key in ('VOL_no'):
...
def fill_out_application_and_submit(driver, random_city, fake_identity):
    ...
    try:
        element_present = expected_conditions.presence_of_element_located((By.ID, 'SurveyControl_SurveySubmit'))
        WebDriverWait(driver, 10).until(element_present)
    except TimeoutException:
        print("Timed out waiting for page to load")
    ...
    try:
        element_present = expected_conditions.presence_of_element_located((By.ID, 'et-ef-content-ftf-gp-j_id_id16pc9-page_0-eSignatureBlock-cfrmsub-frm-dv_cs_esignature_FullName'))
        WebDriverWait(driver, 10).until(element_present)
    except TimeoutException:
        print("Timed out waiting for page to load")


    driver.find_element_by_xpath(FULL_NAME).send_keys(fake_identity['first_name'] + " " + fake_identity['last_name'])
